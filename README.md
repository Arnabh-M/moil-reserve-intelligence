...
|---|---|---|
| **Watcher** | Scheduled 5-minute poll for equipment-down and production-shortfall signals (>15% below target) and for rain forecasts (`weather_advisory_rain` at 35 mm / 24 h, `weather_heavy_rain` at IMD's 64.5 mm; auto-resolved when the forecast clears); idempotent dual-write to Postgres `risk_events` and Neo4j `RiskEvent` with causal edges. | Risk events linked across both stores via `external_ref` |
| **Simulator** | Runs `equipment_down`, `delay_blasting`, or `rainfall_event` (severity in mm of 3-day rain) from the site's real live feature row; only the features a scenario physically touches are perturbed. | `production_forecast_tonnes`, before/after `reserve_confidence`, `affected_graph_path` |
...
- **Model:** XGBoost `XGBRegressor` (pinned `xgboost-cpu==2.1.4`; the trainer refuses any other version and the API refuses a pickle trained under a different one), evaluated on a **time-ordered** 20% holdout against two baselines (train mean, persistence)
- **Result on the holdout (372 site-days):** RMSE 0.0566 vs 0.1168 (train mean) and 0.0844 (persistence); R² gain over persistence +0.30
- **Rainfall is real:** satellite CHIRPS/IMERG rain and SMAP soil moisture, with an Open-Meteo archive fallback (`weather_history.py`) and NaN, never an estimate, when neither has a value
- **Outcome is synthetic:** production and downtime are generated by an explicit causal model (`scripts/synthetic_operations.py`) whose constants are documented assumptions, so what the model learned can be graded against known effects
- **Features (14):** `rain_today_mm`, `rain_3d_mm`, `rain_7d_mm`, `heavy_rain_lag1`, `soil_moisture_m3m3`, `rolling_7d_downtime_pct`, `equipment_down_today_pct`, `days_since_last_maintenance`, `backlog_t`, `blast_delay_days_lag`, plus cyclical day-of-week and month encodings; all history features are machine-checked for look-ahead
- Live inference builds the same feature row from Postgres and the satellite record (`app/services/live_features.py`), and a test checks it equals the training frame
- **Read `docs/MODEL_LIMITATIONS.md` before citing any number from this model**
...
- XGBoost regressor with leakage-safe, time-ordered validation against train-mean and persistence baselines
- Scenario-driven projections consumed by the Simulator agent (sign checked on every day of 2026, see `oresight-backend/docs/simulator_sweep_2026-09-24.json`)
...
- **Shortfall forecaster** — beats both baselines on its holdout, but the outcome it predicts is synthetic (causal constants are our documented assumptions), about 63% of its edge over persistence rides on `blast_delay_days_lag` (synthetic seeded source), and several features have no causal term behind them. The full list, with what we would do with more time, is in [`docs/MODEL_LIMITATIONS.md`](docs/MODEL_LIMITATIONS.md).
