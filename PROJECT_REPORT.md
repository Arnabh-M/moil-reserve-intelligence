...
- 14 engineered features: rainfall (today, 3-day, 7-day, and a heavy-rain flag at
  35 mm on the previous day), soil moisture, rolling 7-day and same-day equipment
  downtime, days since last maintenance, decayed production backlog, blast-delay
  days in the last week, plus cyclical `dow_sin/cos` and `month_sin/cos`
  encodings. All history features are machine-checked to use only data before the
  predicted day.
- Rainfall and soil moisture are real satellite records (CHIRPS/IMERG, SMAP) with
  an Open-Meteo archive fallback and NaN where neither has a value; the production
  outcome is synthetic, generated by an explicit causal model with documented
  assumed effect sizes (see `methodology.md`).
- Evaluated on a time-ordered holdout against train-mean and persistence
  baselines: RMSE 0.0566 vs 0.1168 and 0.0844.
- Exact feature column order persisted for the Simulator agent to reuse; live
  features are checked equal to the training features.
...
- The shortfall forecaster beats both baselines on its holdout, but predicts a
  synthetic outcome whose causal constants are our assumptions, and about 63% of
  its edge over persistence rides on one feature (`blast_delay_days_lag`) with a
  synthetic source. Several features have no causal term in the generator. See
  `docs/MODEL_LIMITATIONS.md` for the complete, honest list.
