  ...
  return (
    <aside className="flex w-72 shrink-0 flex-col border-r border-border bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <Card className="space-y-1 p-0" padded={false}>
          {MAP_LAYERS.map((layer) => (
            <label
              key={layer.id}
              className="flex cursor-pointer items-center gap-3 border-b border-border px-4 py-3 last:border-b-0 hover:bg-bg/60"
            >
              <input
                type="checkbox"
