...
import { Activity, AlertCircle, AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, Bell, Check, ChevronRight, CircleHelp, ClipboardList, CloudUpload, Database, Download, FileText, Gauge, GitBranch, Home, Layers3, MapPin, Loader2, Map as MapIcon, Menu, Moon, RefreshCw, Search, Settings as SettingsIcon, ShieldCheck, SlidersHorizontal, Sun, Truck, Upload, X, Zap } from 'lucide-react';
...
function SimulatorPage() {
  ...
  const [scenario, setScenario] = useState(params.get('scenario_type') || 'equipment_down');
...
function FieldIntakePage() {
  ...
  const [uploadResult, setUploadResult] = useState(null);
  const [toast, setToast] = useState('');
  const [form, setForm] = useState({ site_id: 1, date: '2026-09-05', actual_output: '', target_output: '', note: '' });
  ...
  const onUpload = async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setUploadResult(await api.uploadReport(file)); } catch (uploadError) { showToast(uploadError.status === 409 ? 'Upload conflict: this document was already processed.' : uploadError.detail || 'Upload failed'); } };
  ...
  const submitProduction = async () => {
    if (!form.date || !form.actual_output || !form.target_output || Number(form.actual_output) < 0 || Number(form.target_output) <= 0) { showToast('Validation: enter a date, non-negative actual output, and a target greater than zero.'); return; }
    try { await api.createProduction(form); showToast('Daily production submitted'); setForm({ ...form, actual_output: '', target_output: '', note: '' }); } catch (submitError) { showToast(submitError.status === 409 ? '409 conflict: a production record already exists for this shift.' : submitError.detail || 'Production submission failed'); }
  };
  ...
  return <main className="page"><div className="page-head"><div><div className="eyebrow">Field intake · operations loop</div><h1>Bring the shift into the model</h1><p className="subhead">Capture equipment status, production reality, geology documents, and the notes that explain the deviation.</p></div></div><div className="tabs">{['equipment', 'production', 'geology', 'notes'].map((item) => <button className={`tab ${active === item ? 'active' : ''}`} onClick={() => setActive(item)} key={item} data-testid={`tab-field-${item}`}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>
    {active === 'equipment' && <section className="card section-card"><div className="card-head"><div><div className="card-title">Equipment status</div><div className="card-kicker">contract enum: up / down · reason is required when marking down</div></div><Truck size={17} /></div><div className="table-wrap"><table><thead><tr><th>Site</th><th>Equipment</th><th>Type</th><th>Status</th><th>Last changed</th><th>Reason</th></tr></thead><tbody>{equipmentRows.map((item) => <tr key={item.id}><td>{item.site_name}</td><td><b>{item.name}</b></td><td className="muted">{item.equipment_type}</td><td><select className="select" value={item.status} onChange={(event) => onStatusChange(item, event.target.value)} data-testid={`select-equipment-status-${item.id}`}><option value="up">up</option><option value="down">down</option></select></td><td className="mono">{timeLabel(item.last_status_change)}</td><td><input className="input" value={item.status_reason || ''} placeholder="Reason required when down" onChange={(event) => setEquipmentRows((rows) => rows.map((row) => row.id === item.id ? { ...row, status_reason: event.target.value } : row))} aria-label={`Reason for ${item.name}`} /></td></tr>)}</tbody></table></div></section>}
    {active === 'production' && <section className="card section-card"><div className="card-head"><div><div className="card-title">Daily production entry</div><div className="card-kicker">POST /production · explicit validation and conflict messaging</div></div><span className="pill good">Ready to submit</span></div><div className="form-grid"><div className="field"><label>Site</label><select className="select" value={form.site_id} onChange={(e) => setForm({ ...form, site_id: Number(e.target.value) })} data-testid="select-intake-site"><option value="1">Balaghat</option><option value="2">Nagpur</option><option value="3">Bhandara</option></select></div><div className="field"><label>Shift date</label><input className="input" type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} data-testid="input-shift-date" /></div><div className="field"><label>Actual output (tonnes)</label><input className="input" type="number" min="0" value={form.actual_output} onChange={(e) => setForm({ ...form, actual_output: e.target.value })} placeholder="e.g. 980" data-testid="input-actual-output" /></div><div className="field"><label>Target output (tonnes)</label><input className="input" type="number" min="1" value={form.target_output} onChange={(e) => setForm({ ...form, target_output: e.target.value })} placeholder="e.g. 1040" data-testid="input-target-output" /></div><div className="field full"><label>Shift note</label><textarea className="input" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="What changed from plan?" data-testid="textarea-production-note" /></div></div><button className="btn primary" style={{ marginTop: 18 }} onClick={submitProduction} data-testid="button-submit-production"><Check size={14} /> Submit production</button></section>}
    {active === 'geology' && <section className="card section-card"><div className="card-head"><div><div className="card-title">Geological document upload</div><div className="card-kicker">POST /reports/upload · multipart PDF extraction</div></div><CloudUpload size={18} /></div><label className="upload"><Upload size={24} color="hsl(var(--primary))" /><div style={{ fontWeight: 700, marginTop: 9 }}>Choose a geological PDF</div><div className="subhead">Text and deposit candidates are extracted in mock mode.</div><input type="file" accept="application/pdf" onChange={onUpload} data-testid="input-geology-upload" /></label>{uploadResult && <div className="card section-card" style={{ marginTop: 14 }}><div className="mini-stat"><span><b>{uploadResult.filename}</b><br /><small className="muted">{uploadResult.text_extracted ? 'Text extracted' : 'Text extraction unavailable'}</small></span><span className="pill good">{uploadResult.deposit_count} deposits</span></div><div className="table-wrap"><table><thead><tr><th>Deposit</th><th>Depth</th><th>Grade</th><th>Structure</th><th>Belt zone</th></tr></thead><tbody>{uploadResult.deposits.map((deposit) => <tr key={deposit.deposit_id}><td className="mono">{deposit.deposit_id}</td><td>{deposit.depth} m</td><td>{deposit.grade}%</td><td>{deposit.structure_type}</td><td>{deposit.belt_zone}</td></tr>)}</tbody></table></div><div className="footer-note">{uploadResult.nodes_created.length} causal graph nodes created. {uploadResult.warnings.length ? uploadResult.warnings.join(' ') : 'No extraction warnings.'}</div></div>}</section>}
