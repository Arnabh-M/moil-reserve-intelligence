...
import EquipmentPerformancePanel from './components/EquipmentPerformancePanel';
...
const ManganexLandingPage = lazy(() => import('./pages/landing/ManganexLandingPage'));
...
function Shell({ children }) {
  ...
  useEffect(() => { document.title = `MANGANEX · ${breadcrumbLabel}`; }, [breadcrumbLabel]);
  return <div className="app-shell">
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      <div className="brand">
        ...
        />
        <div>
          <div className="brand-word" style={{ fontSize: 16, letterSpacing: '0.12em', color: '#ffffff', fontWeight: 800 }}>MANGANEX</div>
          <span className="brand-sub" style={{ fontSize: 7.5, letterSpacing: '0.08em', lineHeight: 1.25 }}>
            MAPPING MINERALS<br />FOR A STRONGER TOMORROW
          </span>
        </div>
      </div>
    ...
    <div className="main-col">
      <header className="topbar"><div className="breadcrumb"><button className="btn ghost mobile-toggle" onClick={() => setOpen(!open)} aria-label="Open navigation" data-testid="button-open-navigation"><Menu size={17} /></button><span>MANGANEX</span><ChevronRight size={13} /><strong>{breadcrumbLabel}</strong></div><div className="top-actions"><Link to="/landing" className="btn ghost small" title="MANGANEX Exploration Intelligence" style={{ fontSize: 11, letterSpacing: '0.06em', color: 'hsl(186 100% 45%)', border: '1px solid hsl(186 100% 45% / 0.28)', padding: '3px 9px', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 5 }}><Globe size={12} /> MANGANEX</Link><button className="btn ghost small" title="Toggle theme" onClick={() => setDark(!dark)} data-testid="button-toggle-theme">{dark ? <Sun size={15} /> : <Moon size={15} />}</button><button className="btn ghost small" title="Notifications" data-testid="button-notifications"><Bell size={15} /></button></div></header>
      {children}
    </div>
  </div>;
}
...
// Collapsed-by-default cascade/ripple-impact panel for the Recommendations
// sub-tab's option cards. Renders nothing at all when `cascade` is null/absent
// (a failed or unavailable cascade computation must not change the card).
function CascadePanel({ cascade }) {
  const [expanded, setExpanded] = useState(false);
  if (!cascade) return null;
  return <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid hsl(var(--border))' }}>
    <button type="button" onClick={() => setExpanded((v) => !v)} data-testid="button-toggle-cascade"
      style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', font: 'inherit', color: 'inherit' }}>
      <span className={`pill ${cascadeSeverityClass(cascade.cascade_severity)}`}>Cascade: {cascade.cascade_severity}</span>
      <span className="muted" style={{ fontSize: 10 }}>
        {'· '}
        {cascade.entities_affected === 0
          ? 'no dependent entities found'
          : `${cascade.entities_affected} ${cascade.entities_affected === 1 ? 'entity' : 'entities'} affected`}
        {cascade.blocking_count ? ` · ${cascade.blocking_count} conflict${cascade.blocking_count > 1 ? 's' : ''}` : ''}
      </span>
      <ChevronRight size={12} style={{ marginLeft: 'auto', transform: expanded ? 'rotate(90deg)' : 'none' }} />
    </button>
    {expanded && <div style={{ marginTop: 8 }}>
      <p className="subhead" style={{ margin: '0 0 8px' }}>{cascade.explanation}</p>
      {cascade.impacted.map((entity) => <div className="mini-stat" key={entity.entity_id}>
        <span><b>{entity.name}</b><br /><small className="muted">{entity.entity_type} · {entity.path.join(' → ')}</small></span>
        <span className={`pill ${cascadeClassificationClass(entity.classification)}`}>{entity.classification}</span>
      </div>)}
      {cascade.cascade_adjusted_impact != null && <div className="muted" style={{ fontSize: 10, marginTop: 6 }}>Adjusted impact (cascade-weighted): +{cascade.cascade_adjusted_impact}%</div>}
    </div>}
  </div>;
}
function Recommendation({ recommendation, showToast }) { const option = recommendation.options[0]; if (!option) return <EmptyState title="No graph-sourced alternatives found for this event" />; return <div className="card recommend-card"><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><div><span className="eyebrow">Response to {riskTypeLabel(recommendation.trigger)}</span><h3 style={{ marginTop: 5 }}>{recommendationTypeLabel(option.type)}</h3></div><Zap size={15} color="hsl(var(--primary))" /></div><p>{option.description}</p><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}><span /><div style={{ display: 'flex', gap: 8 }}><ShiftPlanButton riskEventId={recommendation.risk_event_id} option={option} showToast={showToast} testId={`button-apply-dashboard-${recommendation.risk_event_id}`} /><button className="btn small" data-testid={`button-review-${recommendation.risk_event_id}`}>Review <ChevronRight size={12} /></button></div></div></div>; }
...
function Dashboard() {
  ...
  const { kpi, sites, risks, recommendations, production, equipmentMetrics } = data;
  // Soft-failed to null by the client if /equipment/metrics is unavailable —
  // the card degrades to "—" rather than taking the dashboard down.
  const fleet = equipmentMetrics?.fleet ?? null;
  // Production pulse: the LAST 7 dates of the series, summed across sites (the old code showed the first 7 days of one site's 30-day window).
  const byDate = {};
  production.forEach((r) => { const d = (byDate[r.date] = byDate[r.date] || { date: r.date, actual: 0, target: 0 }); d.actual += r.actual_output; d.target += r.target_output; });
  const series = Object.values(byDate).sort((a, b) => (a.date < b.date ? -1 : 1));
  const chartData = series.slice(-7).map((d) => ({ day: dateLabel(d.date), actual: Math.round(d.actual), target: Math.round(d.target) }));
  const dataThrough = series.length ? series[series.length - 1].date : null;
  const dataThroughLabel = dataThrough ? new Date(`${dataThrough}T00:00:00Z`).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC' }) : null;
  // Priority banner: the site whose output was furthest below target over its last 7 days of data (real records only).
  const shortfalls = sites.map((site) => {
    const rows = production.filter((r) => Number(r.site_id) === Number(site.id)).slice(-7);
    const target = rows.reduce((a, r) => a + r.target_output, 0);
    const actual = rows.reduce((a, r) => a + r.actual_output, 0);
    return { site, pct: target > 0 ? (1 - actual / target) * 100 : 0, days: rows.length };
  });
  const worst = shortfalls.reduce((a, b) => (b.pct > (a?.pct ?? -Infinity) ? b : a), null);
  const highSeverity = risks.filter((r) => !r.resolved && ['high', 'critical'].includes(r.severity)).length;
  const twinUpdated = kpi.twin_last_updated ? new Date(kpi.twin_last_updated) : null;
  const twinAgeMin = twinUpdated ? Math.max(0, Math.round((Date.now() - twinUpdated.getTime()) / 60000)) : null;
  const twinAge = twinAgeMin == null ? '' : twinAgeMin < 90 ? `${twinAgeMin} min ago` : twinAgeMin < 2880 ? `${Math.round(twinAgeMin / 60)} h ago` : `${Math.round(twinAgeMin / 1440)} days ago`;
  const filteredRisks = riskFilter === 'all' ? risks : risks.filter((risk) => risk.severity === riskFilter);
  return <main className="page">
    <div className="page-head"><div><div className="eyebrow">{dataThroughLabel ? `Data through ${dataThroughLabel}` : 'Shift briefing'} · shift briefing</div><h1>Morning control room</h1><p className="subhead">A triage view across the Central Indian operating plan. Start with what can move output today.</p></div><div className="filter-row"><button className="btn" onClick={() => window.print()} data-testid="button-print-dashboard"><Download size={14} /> Briefing print</button><Link className="btn primary" to="/simulator?scenario_type=equipment_down&site_id=1&duration_days=7" data-testid="link-open-simulator"><GitBranch size={14} /> Run a scenario</Link></div></div>
    {worst && worst.pct >= 2 && <div className="alert-strip" style={{ marginBottom: 15 }} data-testid="priority-banner"><AlertTriangle size={15} color="hsl(var(--primary))" /><span><strong>Priority:</strong> {worst.site.name} output averaged {worst.pct.toFixed(1)}% below target over its last {worst.days} days of data{dataThroughLabel ? ` (through ${dataThroughLabel})` : ''}.</span><Link to={`/site/${worst.site.id}?tab=recommendations`} style={{ marginLeft: 'auto', color: 'hsl(var(--primary))', fontWeight: 700 }}>Open signal <ChevronRight size={13} style={{ verticalAlign: 'middle' }} /></Link></div>}
    <div className="stat-grid five"><StatCard label="Active risk events" value={kpi.active_risk_events} foot={`${highSeverity} high or critical`} icon={AlertTriangle} /><StatCard label="Avg reserve confidence" value={percent(kpi.avg_reserve_confidence, 1)} foot="across active zones" icon={ShieldCheck} /><StatCard label="Fleet availability" value={fleet ? `${fleet.physical_availability_pct}%` : '—'} foot={fleet ? `${fleet.maintenance_overdue_count} maintenance overdue` : 'metrics unavailable'} icon={Truck} /><StatCard label="Sites under watch" value={kpi.sites_under_watch} foot={`of ${sites.length} operating sites`} icon={Activity} /><StatCard label="Twin last updated" value={twinUpdated ? timeLabel(twinUpdated) : '—'} foot={twinUpdated ? `${dateLabel(twinUpdated)} · ${twinAge}` : 'no recorded change'} icon={RefreshCw} /></div>
...
function SitePage() {
  ...
  const [toast, setToast] = useState('');
  const tabs = ['overview', 'production', 'equipment', 'reserve', 'recommendations', 'graph'];
  ...
  return <main className="page"><div className="site-banner"><div><div className="eyebrow">Site intelligence · {site.id}</div><h1>{site.name}</h1><div className="site-place">{site.belt_name} · {site.district}, {site.state}</div></div><div className="filter-row"><span className={`pill ${site.active_risk_count > 2 ? 'critical' : 'warn'}`}>{site.active_risk_count} active risks</span><Link className="btn small" to="/map">View on map <MapIcon size={12} /></Link></div></div><div className="tabs">{tabs.map((item) => <button className={`tab ${tab === item ? 'active' : ''}`} onClick={() => setTab(item)} key={item} data-testid={`tab-site-${item}`}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>
    ...
    {tab === 'equipment' && <EquipmentPerformancePanel siteId={site.id} />}
...
function ReportsPage() {
  ...
  const exportCsv = () => { const csv = ['site,risk,severity,score,detected_at', ...risks.map((r) => `${r.site_name},${r.risk_type},${r.severity},${r.score},${r.detected_at}`)].join('\n'); const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'manganex-risk-register.csv'; anchor.click(); URL.revokeObjectURL(url); setToast('CSV report generated'); setTimeout(() => setToast(''), 2200); };
...
function LoginPage() { const navigate = useNavigate(); const [email, setEmail] = useState('anika.kulkarni@manganex.in'); return <div className="login-page"><section className="login-art"><div className="brand"><img src="/manganex_emblem_compact.svg" alt="MANGANEX" style={{ width: 36, height: 36, flexShrink: 0, borderRadius: 8 }} /><div><div className="brand-word" style={{ letterSpacing: '0.12em' }}>MANGANEX</div><span className="brand-sub">Mapping Minerals for a Stronger Tomorrow</span></div></div><div><div className="eyebrow">Operations intelligence · India</div><h1>See the plan<br />beneath the ground.</h1><p>One control room for reserve confidence, production drift, and the decisions that keep a shift moving.</p><div className="login-pattern" /></div><div className="muted" style={{ fontSize: 10 }}>Preview workspace · v0.9.4</div></section><section className="login-form-side"><form className="login-form" onSubmit={(e) => { e.preventDefault(); navigate('/'); }}><div className="eyebrow">Planner sign in</div><h2>Welcome back.</h2><p className="subhead">Continue to your operating workspace.</p><div className="field"><label>Work email</label><input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} data-testid="input-login-email" /></div><div className="field"><label>Access key</label><input className="input" type="password" defaultValue="preview-access" data-testid="input-login-password" /></div><button className="btn primary" type="submit" data-testid="button-login">Enter MANGANEX <ChevronRight size={14} /></button><div className="footer-note">Visual-only sign in for the preview environment. No credentials are transmitted.</div></form></section></div>; }
function NotFound() { return <main className="page" style={{ minHeight: '100dvh', display: 'grid', placeItems: 'center' }}><div style={{ textAlign: 'center', maxWidth: 420 }}><img src="/manganex_emblem_compact.svg" alt="MANGANEX" style={{ width: 44, height: 44, margin: '0 auto 17px', display: 'block', borderRadius: 10 }} /><div className="eyebrow">404 · outside the mine plan</div><h1 style={{ marginTop: 10 }}>This page is not mapped.</h1><p className="subhead">The route does not exist in the current MANGANEX workspace.</p><Link className="btn primary" to="/" style={{ marginTop: 20 }}>Return to dashboard <Home size={13} /></Link></div></main>; }
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route
          path="/landing"
          element={
            <Suspense fallback={<div style={{ minHeight: '100vh', background: '#07080a', display: 'grid', placeItems: 'center', color: '#00f0ff', fontFamily: 'monospace' }}>MANGANEX INITIALIZING...</div>}>
              <ManganexLandingPage />
            </Suspense>
          }
        />
