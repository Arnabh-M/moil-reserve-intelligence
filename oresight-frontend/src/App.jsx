...
function Dashboard() {
  ...
  return <main className="page">
    ...
    <div className="stat-grid"><StatCard label="Active risk events" value={kpi.active_risk_events} foot={`${highSeverity} high or critical`} icon={AlertTriangle} /><StatCard label="Avg reserve confidence" value={percent(kpi.avg_reserve_confidence, 1)} foot="across active zones" icon={ShieldCheck} /><StatCard label="Sites under watch" value={kpi.sites_under_watch} foot={`of ${sites.length} operating sites`} icon={Activity} /><StatCard label="Twin last updated" value={twinUpdated ? timeLabel(twinUpdated) : '—'} foot={twinUpdated ? `${dateLabel(twinUpdated)} · ${twinAge}` : 'no recorded change'} icon={RefreshCw} /></div>
